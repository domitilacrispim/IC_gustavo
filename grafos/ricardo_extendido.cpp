...
int main (int argc,  char *argv[ ] ){
	...
	char nom[50], cat[100], nom_p[50], car, dire[100]="\0", sequencia[200][200];
	populacao p;
	FILE *arq, *arq2;	
		...
		while(fscanf(arq, "%s", nom)!=EOF){
			...
			for(int jo=0; jo<tam; jo++){
				if(strcmp(g.vertices[jo].nome, nom)==0){
						for ( int juju=0; juju<tam_seq; juju++){
							for(int iter=0; iter<tam; iter++){
								if(strcmp(g.vertices[iter].nome, sequencia[juju])==0){
									g.vertices[jo].aparicao++;
									g.vertices[ji].vizinho[(int)g.vertices[ji].total ] = iter;
									g.vertices[ji].total++;
								}
							}		
						}
						strcpy(sequencia[tam_seq], nom);
						g.vertices[jo].aparicao++;
