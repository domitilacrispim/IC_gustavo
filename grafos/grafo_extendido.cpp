...
int main (int argc,  char *argv[ ] ){
	...
	Grafo g;
	int tam, i=0, ji, max=0, min, bal=0, ju, vet[1000], tam_vet=0;
		...
		while(fscanf(arq, "%s", nom)!=EOF){
			for(int jo=0; jo<tam; jo++){
				if(strcmp(g.vertices[jo].nome, nom)==0){
						for(ji=0; ji<tam_vet; ji++){
							g.vertices[vet[ji]].vizinho[(int)g.vertices[vet[ji]].total ]=jo;			
							g.vertices[vet[ji]].total++;
						}
						vet[tam_vet]=jo;
						tam_vet++;
						break;
